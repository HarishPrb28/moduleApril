...
async function getPostalData(pin) {
  return await fetch(`https://api.postalpincode.in/pincode/${pin}`).then(
...
document.getElementById("getData").addEventListener("click", (e) => {
  ...
  getData().then(() => {
    ...
    document.getElementById("date").innerHTML += getCurrentTime(
      ipData.timezone
    );
...
document.getElementById("search").addEventListener("input", (e) => {
  ...
  renderItems(
    postalData.PostOffice.filter(
      (i) =>
        i.Name.toLowerCase().includes(input) ||
        i.BranchType.toLowerCase().includes(input)
    )
  );
});
